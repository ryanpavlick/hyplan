...
![Wind-aware mission planning for the same Rincón de la Vieja survey using a King Air B-200. (a) Mission map; solid lines are still-air Dubins arcs and dashed lines are trochoidal arcs under a 60 kt northeasterly wind, both color-coded by phase. (b) Altitude vs. elapsed mission time, both conditions overlaid. (c) Per-phase time totals (still vs. wind) for climb/descent, transit, and data collection, with absolute deltas annotated. Wind redistributes time across phases — transit arcs distort while data-collection legs see crab-corrected groundspeed effects — even when total mission time changes only modestly, an effect that propagates into fuel planning and airspace coordination.\label{fig:wind}](figures/fig2_wind_dubins.png)
