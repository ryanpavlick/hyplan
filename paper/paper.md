...
**Airport selection** leverages the OurAirports global database, filtering by proximity, country, airport type, runway length, and surface type, and exposing runway dimensions, headings, and elevation for operational matching against a given aircraft.

**Airspace conflict detection** checks planned flight lines against controlled airspace, temporary flight restrictions, and special-use airspace using OpenAIP for international boundaries and FAA TFR/NASR for US-specific restrictions, reporting intersections with airspace class, altitude floors and ceilings, and effective schedules so that conflicts are resolved pre-campaign rather than during flight operations.

**Satellite overpass prediction** supports coordination of airborne observations with 15 missions including PACE, Landsat-8/9, Sentinel-2A/B, Sentinel-3A/B, JPSS-1/2, Aqua, Terra, ICESat-2, CALIPSO, CloudSat, and EarthCARE. The module fetches TLE sets from CelesTrak, propagates orbits with Skyfield [@rhodes2019skyfield], and returns per-overpass ground tracks and swath footprint polygons filtered by solar zenith angle and spatial overlap with the study area.
...
Early versions of HyPlan have been used to support flight planning for NASA airborne science campaigns including BioSCape [@cardoso2025bioscape], SHIFT [@chadwick2025shift], and the 2022--2023 ABoVE AVIRIS campaigns [@miller2025above].

Because every planning decision is a Python function call, complete campaign plans are captured in version-controlled notebooks that can be re-executed to reproduce the exact flight lines, swaths, and schedules delivered to pilots and science teams. In the campaigns listed above, this has made it possible to audit altitude and coverage decisions, re-plan when study-area boundaries or sensor configurations changed, and share planning methodology across instrument teams.

All outputs derive from a single flight-plan GeoDataFrame and export to audience-specific formats: ForeFlight CSV, Honeywell FMS, and GPX for pilots; ICARTT and Excel for science teams; GeoJSON/KML for archival and GIS; and interactive Folium web maps and matplotlib altitude profiles [@hunter2007matplotlib] for visual review.

Quality assurance is supported by over 1,200 automated tests covering more than 80% of the codebase, run via continuous integration on every commit; a full API reference on GitHub Pages; and over 20 Jupyter notebooks that serve as both tutorials and integration tests. A dedicated validation notebook checks HyPlan's core calculations against independent references---Vincenty's published geodesic test cases, NOAA's solar calculator, and analytical GSD and swath-width solutions.

Adding new aircraft or sensors requires only specifying the performance envelope or sensor parameters, positioning HyPlan to support the broader airborne science community beyond the NASA instruments and aircraft currently included.
