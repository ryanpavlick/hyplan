...
Realistic aircraft maneuvering between waypoints is modeled using 3D Dubins curves [@dubins1957curves; @walker2011dubins; @vana2020dubins3d]: the shortest path between two oriented points subject to a minimum turning radius $R = v^2 / (g \tan\phi)$ derived from true airspeed $v$ and maximum bank angle $\phi$, coupled with the aircraft's climb and descent profiles to produce a full 3D trajectory with bounded pitch angle between waypoints at different altitudes. In still air the horizontal path is a combination of circular arcs and straight segments; under wind the arcs become trochoidal ground tracks (circles drifting with the wind), solved in the air-relative frame following Sachdev et al. [@sachdev2023trochoid] and mapped to ground coordinates with the cumulative wind drift. The solution also yields the crab angle---the offset between aircraft heading and ground track---which affects both mission timing and the orientation of the sensor's cross-track field of view relative to the ground. \autoref{fig:dubins} compares still-air and wind-perturbed trajectories for a complete mission from Liberia airport (MRLB) to the Rincón de la Vieja flight lines. To our knowledge, HyPlan is the only open-source flight planning tool that implements wind-aware trochoidal 3D Dubins paths for airborne science applications.
