...
- **Heterogeneous visit-item optimizer** (`hyplan.flight_optimizer.greedy_optimize`): the `flight_lines` argument now accepts a mixed list of `FlightLine | Pattern | Waypoint` objects. The output `flight_sequence` is a list of the same heterogeneous kinds in scheduled order.
  ...
  - **Result schema**: new `items_covered` and `items_skipped` keys on the `greedy_optimize` result count visit items in the heterogeneous-input sense (each Pattern/FlightLine/Waypoint is one item). The existing `lines_covered` and `lines_skipped` keys are preserved with their pre-v1.2 semantics — `lines_covered` reports actual flight-line legs (a 3-leg line-based Pattern contributes 3, a Waypoint contributes 0), and `lines_skipped` is an alias of `items_skipped`. For all-FlightLine input the two pairs are equivalent.
