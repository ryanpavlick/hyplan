...
for p in sorted(DATA_DIR.glob(TAIL_GLOB)):
    ...
    if peak > MAX_PEAK_ALT_FT:
        skipped.append((p.stem, f"high peak alt ({peak:.0f} ft) — not a P-3"))
        continue
...
climb_pts = [(0, ROTATION_TAS_KT)] + schedule_pts(
    climb_tas, [5000, 10000, 15000, 20000, 25000]
)
...
# level-offs.
cruise_pts = schedule_pts(cruise_tas, [15000, 20000, 25000, 28000])

# Descent: low-altitude anchor at typical approach TAS, then
# descent-phase medians up to the typical cruise ceiling.
descent_pts = schedule_pts(descent_tas, [5000, 10000, 15000, 20000, 25000])
