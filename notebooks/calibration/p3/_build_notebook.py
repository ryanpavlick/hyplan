...
DATA_DIR = Path("../../../data/p3").resolve()
TAIL_GLOB = "*_*.txt"  # in-house p3_*.txt + ASP archive n426_*.txt
...
MAX_DUR_MIN     = 900.0  # P-3 sorties run up to ~14 hr; cap above that
MIN_PEAK_ALT_FT = 10000  # P-3 cruises low; FL100 is enough to be a real sortie
MAX_PEAK_ALT_FT = 32000  # P-3 brochure service ceiling; tail N426NA has
                         # been reassigned to different aircraft over the
                         # years — sorties peaking above 32 kft are
                         # mislabeled (likely a jet under the same tail).
""")
...
ax.set_title(f"NASA 426 P-3 altitude profiles — {len(sorties)} sorties")
ax.grid(alpha=0.3)
ax.set_ylim(0, 50)
plt.tight_layout()
_show_plot()
...
code(r"""
CEILING_FT     = 30000           # P-3 service ceiling (brochure)
...
climb_pts = [(0, ROTATION_TAS_KT)] + schedule_pts(
    climb_tas, [5000, 10000, 15000, 20000, 25000], n_min=200
)

# Cruise: P-3 cruises low — typical band FL150-FL250.  Anchor on those
# altitudes; below FL150 the cruise-phase bin is mostly transient
# level-offs.
cruise_pts = schedule_pts(cruise_tas, [15000, 20000, 25000, 28000], n_min=200)

# Descent: low-altitude anchor at typical approach TAS, then
# descent-phase medians up to the typical cruise ceiling.
descent_pts = schedule_pts(descent_tas, [5000, 10000, 15000, 20000, 25000], n_min=200)
...
that follow describe **operational** behavior across the n=*
sortie set: wall-clock time-to-FL200 includes pre-cruise
level-offs and ATC routing; per-sortie peaks reflect the actual
mission profiles flown rather than the airframe service ceiling
under MTOW; approach TAS is the median final-approach speed for
this mission mix.  Aircraft-intrinsic performance (climb / descent
/ cruise schedules in §5–§7, bank in §8) is what the planner
consumes; the §9 numbers are reviewer-facing context.
""")
...
code(r"""
# TOC = takeoff -> first FL200 fix (P-3 typical cruise band).
toc = []
for a in sorties.values():
    above = a[a["altitude"] >= 20000]
    if not above.empty:
...
print(f"TOC (takeoff -> FL200):  n={len(toc_s)}, median {toc_s.median():.1f} min, IQR {toc_s.quantile(.25):.1f}-{toc_s.quantile(.75):.1f}, range {toc_s.min():.1f}-{toc_s.max():.1f}")
...
`NASA_P3()` — every commit makes the two trivially identical, and
...
code(r"""
print("# Paste into hyplan/aircraft/_models.py NASA_P3.__init__")
print()
...
print(f'    reference="NASA 426 IWG1 calibration, n={len(sorties)} sorties",')
